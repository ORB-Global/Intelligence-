...
async function assembleSharedIntelligence(supabase, locationId) {
  const [
    ...
    supabase.from('social_posts').select('caption, permalink, image_url, image_url_synced_at, likes, comments, shares, clicks, engagement_rate, created_at').eq('location_id', locationId).order('clicks', { ascending: false }).limit(5),
