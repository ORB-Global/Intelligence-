...
async function buildTenantChatContext(supabase, locationId) {
  const [
    ...
    supabase.from('competitors').select('name, address, category, status, confidence, seo_visibility_data, paid_search_data, provider_enriched_at, same_brand, domain').eq('location_id', locationId),
