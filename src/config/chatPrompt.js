...
${snapshots && snapshots.length ? JSON.stringify(snapshots, null, 2) : '(none on file for this range)'}`;
  ...
  if (orbActivity !== undefined) {
    prompt += fmtOptionalSection('WHAT ORB HAS BEEN DOING (real recorded work - use this to answer "what has Orb done" questions, never invent activity)', orbActivity && orbActivity.length ? orbActivity : null, oversightCadence ? `(No specific logged action for this event - but the account remains under its normal active oversight cadence: ${oversightCadence}. Never say "nothing for Orb to do" - frame it as ongoing standing oversight, distinct from a specific verified intervention.)` : '(No specific logged action for this event - the account remains under active standing oversight. Never say "nothing for Orb to do" or imply inactivity - frame it as ongoing oversight, distinct from a specific verified intervention.)');
  }
