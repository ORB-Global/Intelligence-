...
${JSON.stringify(client, null, 2)}`;
  ...
  if (topPosts !== undefined) {
    prompt += fmtOptionalSection('REAL TOP ORGANIC POSTS (this business\'s own real Facebook posts, ranked by real clicks - each has a real caption, real likes/comments/shares/clicks/engagement_rate, a real permalink URL, and a real image_url (a direct Facebook CDN link to the actual post image). Use this directly when asked "which post did best," "what content works," or similar - cite the real caption and real numbers. image_url can be shown/linked directly when relevant - it is a genuine, working image link, not a placeholder)', topPosts && topPosts.length ? topPosts : null, '(no real post data connected yet)');
  }
